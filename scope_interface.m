% need to add delay after displaying functions
...
function initialize(ip_address)
 %% Might want to clear markers here
 global initialized;
 global v; % Make sure this works for all other functions
...
% Below functions will detail saving channel data to wave memory and
% applying functions to different aspects in wave memory
% Helpful functions
% :SYSTem:CAPability:CHANnel? queries the number of channels that in use
% :ANALyze:SIGNal:TYPE allows one to specify what signal is (spectral for I/Q)
% Also default signal type is NRZ signal
% :FUNCtion<F>:ADEMod demodulates amplitude modulated signals (prob not good)
% above function taks sqrt(I^2 + Q^2), stick to analyze signal type
% :MEASure:BER? might work for qam?
% :MTESt:FOLDing might work for real-time eye?
% For eye diagram, check mask functions in scope gui
% also use NRZ signal for eye diagram
% Goal for week of 7/22 is to be able to demodulate QPSK signal
% This should be accomplished by setting signal type to spectral
% Also note that when a line is queried, one needs to read it right after
% in order to clear it from the memory buffer
% another goal is to determine teh best way to store the data (wvememory?)
% Need to also think of general functionality
% 1. FFT
% 2. IQ
% 3. Eye diagram -- multiple ways, mocked 1
% 4. waveform generation -- CSV file, similar to 430
% 5. BER -- Need to check scope function, can do without, need PAM signal
% 6. SNR -- not mocked, but function is available in scope (measure sndr)
% 7. Markers/peaks -- in scope, done in example
% 8. MIMO possible (i.e. adding signals) -- in functions for adding
% 9. PSD -- in measure
% 10. Bandwidth -- in measure
function set_data_format(dformat)
 ...
 %disp([':WAVEFORM:FORMAT ', tmp_format])
...
function iq_data = get_iq(source)
 ...
 %disp([':ANALyze:SIGNal:TYPE ', source,', SPECTRAL'])
